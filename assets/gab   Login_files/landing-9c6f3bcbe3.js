...
return Promise.reject(reason);});};},{"../cancel/isCancel":6,"../defaults":14,"./../helpers/combineURLs":18,"./../helpers/isAbsoluteURL":20,"./../utils":25,"./transformData":13}],11:[function(require,module,exports){'use strict';module.exports=function enhanceError(error,config,code,request,response){error.config=config;if(code){error.code=code;}
...
function isSlowBuffer(obj){return typeof obj.readFloatLE==='function'&&typeof obj.slice==='function'&&isBuffer(obj.slice(0,0))}},{}],27:[function(require,module,exports){/*!
...
process.binding=function(name){throw new Error('process.binding is not supported');};process.cwd=function(){return '/'};process.chdir=function(dir){throw new Error('process.chdir is not supported');};process.umask=function(){return 0;};},{}],30:[function(require,module,exports){module.exports='<div class="auth__signup">\n    <h1>Join Gab</h1>\n    <p class="bigger">Gab is a growing community of people from around the world who support free speech online. In order to prevent the spam and bot accounts that are so prevalent on other social networks, Gab requires an invite code to join. We care about creating a community of real people having real conversations. </p>\n    <p class="bigger">You can still explore Gab without an account, but you’ll need an invite in order to create content and follow others.</p>\n    <a href="#" class="auth__link" @click.prevent="showInvitationForm = true"><i class="fa fa-chevron-right"></i>I have an invitation code</a>\n    <!--<a href="#" class="auth__link" @click.prevent="showWishlistForm()"><i class="fa fa-chevron-right"></i>Add me to waitlist</a>-->\n    <form v-if="showInvitationForm" @submit.prevent="submitInvitation()" method="post">\n    	<label for="name">Invitation Code</label>\n\n    	<div class="row">\n    		<div class="col-md-9">\n		    	<div class="form-group">\n		    	    <div class="auth__input-icon">\n		    	        <input id="invitation_id" name="invitation_id" type="text" class="form-control" v-model="form.invitation_id">\n		    	        <i class="fa fa-times" v-if="errors[\'invitation_id\'] && errors[\'invitation_id\'].length > 0"></i>\n		    	    </div>\n		    	    <div class="auth__error"><div v-for="error in errors[\'invitation_id\']">{{ error }}</div></div>\n		    	</div><!--.form-group-->\n		    </div><!--.col-md-9-->\n\n		    <div class="col-md-3">\n		    	<button type="submit" class="auth__signup__inline-button">\n		    		<span v-if="invitationLoading"><i class="fa fa-sync fa-spin"></i></span>\n		    		<span v-else>Submit</span>\n		    	</button>\n		    </div><!--.col-md-3-->\n	    </div><!--.row-->\n    </form>\n</div>';},{}],31:[function(require,module,exports){'use strict';var InvitationRequired={props:['email'],template:require('./InvitationRequired.html'),data:function data(){return{errors:{},invitationLoading:false,showInvitationForm:false,form:{email:this.email,invitation_id:""}};},methods:{submitInvitation:function submitInvitation(){var _this=this;this.invitationLoading=true;axios.post('/api/invitations/validate',{invitation_id:this.form.invitation_id}).then(function(response){window.location.href="/auth/register?email="+_this.email+"&invitation_id="+_this.form.invitation_id;}).catch(function(e){_this.invitationLoading=false;_this.errors=e.response.data.errors;});}}};module.exports=InvitationRequired;},{"./InvitationRequired.html":30}],32:[function(require,module,exports){module.exports='<form @submit.prevent="submit()" method="post" class="auth__signup">\n    <input style="display:none" type="text" name="avoid-username"/>\n\n    <h1>Join Gab</h1>\n    <p class="bigger">Gab defends individual liberty, free expression, and the free flow of information. All are welcome to join and speak freely.</p>\n\n    <div class="auth__error"><div v-for="error in errors[\'invitation\']">{{ error }}</div></div>\n\n    <div class="form-group">\n        <label for="name">Full Name</label>\n        <div class="auth__input-icon">\n            <input id="name" name="name" type="text" class="form-control" v-model="form.name" @blur="validateRegister(\'name\')">\n            <i class="fa fa-times" v-if="errors[\'name\'] && errors[\'name\'].length > 0"></i>\n        </div>\n        <div class="auth__error"><div v-for="error in errors[\'name\']">{{ error }}</div></div>\n    </div><!--.form-group-->\n\n    <div class="form-group">\n        <label for="username">Username</label>\n        <div class="auth__input-icon">\n            <input id="username" name="username" type="text" class="form-control" v-model="form.username" @input="validateRegister(\'username\')" @blur="validateRegister(\'username\')" autocomplete="avoid-username">\n            <i class="fa fa-times" v-if="errors[\'username\'] && errors[\'username\'].length > 0"></i>\n        </div>\n        <div class="auth__error"><div v-for="error in errors[\'username\']">{{ error }}</div></div>\n    </div><!--.form-group-->\n\n    <div class="form-group">\n        <label for="email">Email</label>\n        <div class="auth__input-icon">\n            <input id="email" name="email" type="text" class="form-control" v-model="form.email" @blur="validateRegister(\'email\')">\n            <i class="fa fa-times" v-if="errors[\'email\'] && errors[\'email\'].length > 0"></i>\n        </div>\n        <div class="auth__error"><div v-for="error in errors[\'email\']">{{ error }}</div></div>\n    </div><!--.form-group-->\n\n    <div class="form-group">\n        <label for="password">Password</label>\n        <div class="auth__input-icon">\n            <input id="password" name="password" type="password" class="form-control" v-model="form.password" @blur="validateRegister(\'password\')">\n            <i class="fa fa-times" v-if="errors[\'password\'] && errors[\'password\'].length > 0"></i>\n        </div>\n        <div class="auth__error"><div v-for="error in errors[\'password\']">{{ error }}</div></div>\n    </div><!--.form-group-->\n\n	<vue-recaptcha ref="recaptcha" @verify="recaptchaVerify" @expired="recaptchaExpired" sitekey="6Lf-0U4UAAAAADvmry8SJ9ceqWGnPDIj5AAj5O9h"></vue-recaptcha>\n    <div class="auth__error" v-if="errors[\'g-recaptcha-response\'] && errors[\'g-recaptcha-response\'].length > 1">{{ errors[\'g-recaptcha-response\'][1] }}</div>\n\n    <button type="submit">\n    	<span v-if="loading"><i class="fa fa-sync fa-spin"></i></span>\n    	<span v-else>Create Account</span>\n    </button>\n\n    <p class="center">By clicking \'Create Account,\' you agree to the <a href="/about/tos">Terms of Service</a>.</p>\n</form><!--.auth__signup-->';},{}],33:[function(require,module,exports){'use strict';function _defineProperty(obj,key,value){if(key in obj){Object.defineProperty(obj,key,{value:value,enumerable:true,configurable:true,writable:true});}else{obj[key]=value;}return obj;}
var Register={props:['email','invitation_id'],components:{VueRecaptcha:VueRecaptcha},template:require('./Register.html'),data:function data(){return{errors:{},loading:false,loadingUsernameAvailability:false,usernameAvailable:null,touched:{email:false,password:false,username:false,name:false},form:{invitation_id:this.invitation_id,email:this.email,password:"",username:"",name:"","g-recaptcha-response":""}};},methods:{submit:function submit(){var _this=this;this.loading=true;axios.post('/auth/signup',this.form).then(function(response){window.location.href="/home?onboarding=true";}).catch(function(e){_this.loading=false;_this.errors=e.response.data.errors;_this.$refs.recaptcha.reset();});},recaptchaVerify:function recaptchaVerify(response){this.form=Object.assign({},this.form,{"g-recaptcha-response":response});},recaptchaExpired:function recaptchaExpired(){this.form=Object.assign({},this.form,{"g-recaptcha-response":""});},validateRegister:_.debounce(function(field){var _this2=this;axios.post('/auth/register-validation',this.form).then(function(){_this2.errors={};}).catch(function(e){_this2.errors=Object.assign({},_this2.errors,_defineProperty({},field,e.response.data.errors[field]));});},300)}};module.exports=Register;},{"./Register.html":32}],34:[function(require,module,exports){'use strict';window._=require('lodash');window.$=window.jQuery=require('jquery');window.axios=require('axios');window.axios.defaults.headers.common['X-Requested-With']='XMLHttpRequest';window.axios.defaults.headers.common['X-CSRF-TOKEN']=document.head.querySelector('meta[name="csrf-token"]').content;Vue.component('register',require('./components/Register.js'));Vue.component('invitation-required',require('./components/InvitationRequired.js'));var positionHome=function positionHome(){var headerHeight=$('.header').height();var footerHeight=$('.footer').height();var frameHeight=$(window).height();$('.static-home-wrapper').height(Math.max(600,frameHeight-headerHeight-footerHeight));};$(window).resize(_.debounce(positionHome,200));positionHome();setTimeout(function(){return $('.landing-container--bg-2').addClass('active');},100);var app=new Vue({el:'#app'});},{"./components/InvitationRequired.js":31,"./components/Register.js":33,"axios":1,"jquery":27,"lodash":28}]},{},[34]);